                ...
                "luckynetwork" -> {
                    if (clickState == 0 && windowId == 0 && slot == 44 && itemName.contains("bed", ignoreCase = true)) {
                        if (isLNRejoining == false) {
                           ...
                           isLNRejoining = true
                           LiquidBounce.hud.addNotification(Notification(this.name, "Trying to send you to an game", NotifyType.INFO))
                        }
                    }
                }
            }
        ...
        when (modeValue.get().lowercase()) {
            "luckynetwork" -> {
                if (isLNRejoining == true) {
                    ...
                    isLNRejoining = false
                    LiquidBounce.hud.addNotification(Notification(this.name, "Joined the game.", NotifyType.INFO))
                }
            }
        }
    }
