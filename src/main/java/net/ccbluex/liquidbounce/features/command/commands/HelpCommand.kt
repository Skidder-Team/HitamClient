...
class HelpCommand : Command("help", emptyArray()) {
    ...
    override fun execute(args: Array<String>) {
        ...
        while (i < 8 * page && i < commands.size) {
            ...
            ClientUtils.displayChatMessage("§b-> §7${LiquidBounce.commandManager.prefix}${command.command}${if (command.alias.isEmpty()) "" else " §7(§8" + Strings.join(command.alias, "§7, §8") + "§7)"}")
            i++
        }
